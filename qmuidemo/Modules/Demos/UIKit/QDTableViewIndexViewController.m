//
//  QDTableViewIndexViewController.m
//  qmuidemo
//
...
@interface QDTableViewIndexViewController ()

@property(nonatomic, strong) QDTableViewInsetDebugPanelView *debugView;
@end

@implementation QDTableViewIndexViewController
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    NSInteger tag = 111;
    QMUIButton *button = [headerView viewWithTag:tag];
    if (!button) {
    ...
    button.qmui_right = headerView.qmui_width - ([headerView isKindOfClass:[QMUILabel class]] ? ((QMUILabel *)headerView).contentEdgeInsets.right : 24);
    button.qmui_top = CGFloatGetCenter([tableView.delegate tableView:tableView heightForHeaderInSection:section], button.qmui_height);
    
    if ([headerView isKindOfClass:[UILabel class]]) {
        ((UILabel *)headerView).font = UIFontMake(15);
    }
    return headerView;
}
