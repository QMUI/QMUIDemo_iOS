...
- (void)handleCustomDimmingView {
    ...
    UIImage *blurredBackgroundImage = [UIImage imageWithView:self.navigationController.view];
...
- (void)handleKeyboard {
    ...
    UILabel *label = [[UILabel alloc] init];
    label.numberOfLines = 0;
    NSMutableParagraphStyle *paragraphStyle = [NSMutableParagraphStyle paragraphStyleWithLineHeight:20];
    ...
    NSDictionary *codeAttributes = @{NSFontAttributeName: CodeFontMake(12), NSForegroundColorAttributeName: [UIColorBlue colorWithAlphaComponent:.8]};
    [attributedString.string enumerateCodeStringUsingBlock:^(NSString *codeString, NSRange codeRange) {
        [attributedString addAttributes:codeAttributes range:codeRange];
    }];
...
- (void)viewDidLayoutSubviews {
    ...
    if (IS_LANDSCAPE) {
        ...
        [self.imageView sizeToFitKeepingImageAspectRatioInSize:CGSizeMake(imageViewLimitWidth, CGFLOAT_MAX)];
